...
\begin{align*}
D(y;\hat{\mu})&=2 \sum_{i=1}^n\left[\frac{Y_i\ln\left(\frac{Y_i}{1-Y_i}\right)+\ln(1-Y_i)}{1/m_i}-\frac{Y_i\ln\left(\frac{\hat{\mu}_i}{1-\hat{\mu}_i}\right)+\ln(1-\hat{\mu}_i)}{1/m_i}\right]\\
&=2 \sum_{i=1}^n m_i\left[Y_i\ln\left(\frac{Y_i}{\hat{\mu}_i}\right)+(1-Y_i)\ln\left(\frac{1-Y_i}{1-\hat{\mu}_i}\right)\right].
\end{align*}
\end{sol}
\end{exercice}

\begin{exercice}
Trouver les expressions des résidus de Pearson, d'Anscombe et de déviance pour la loi Gamma en utilisant le lien canonique.

\begin{rep}
Pearson : $r_{P_i} = (Y_i-\hat{\mu}_i)/\hat{\mu}_i $, 
Anscombe : $r_{A_i} = (3(Y_i^{1/3}-\hat{\mu}_i^{1/3}))(\hat{\mu}_i^{1/3})$,
Déviance : $r_{D_i}=signe(Y_i-\hat{\mu}_i)\sqrt{2\left(\ln\left(\frac{\hat{\mu}_i}{Y_i}\right)+\frac{Y_i-\hat{\mu}_i}{\hat{\mu}_i}\right)}$
\end{rep}
